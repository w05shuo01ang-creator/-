...
assert.strictEqual(parseArgs(['images', '--default-tag', '基础表情']).defaultTag, '基础表情')
assert.strictEqual(parseArgs(['images', '--no-tags']).noTags, true)
assert.throws(() => parseArgs(['images', '--default-tag', '基础表情', '--no-tags']), /不能同时使用/)
assert.throws(() => parseArgs(['images', '--default-tag', '这是一个超过十个字符的统一标签']), /1 到 10 个字符/)
assert.strictEqual(parseArgs(['images', '--offset', '100']).offset, 100)
assert.throws(() => parseArgs(['images', '--offset', '-1']), /非负整数/)
