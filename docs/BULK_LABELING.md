...
推荐把 `memecraft-labels.json` 和对应图片压缩成一个 ZIP，再把 ZIP 发送到微信文件传输助手。小程序会在本地解压、校验文件名和完整性，上传结束后自动删除临时解压目录。这样不需要把聊天图片逐张选择为文件。

1. 在“我的表情”页点击管理员专用的“批量”按钮。
2. 选择包含清单和图片的 ZIP，单批最多 100 张图片，ZIP 最大 100 MB。
3. 确认后保持页面打开；小程序会逐张上传并自动提交微信内容审核。

原有的“两次选择”方式仍可使用：先选择 JSON，再选择以文件形式发送且保留原文件名的 JPG/PNG。
