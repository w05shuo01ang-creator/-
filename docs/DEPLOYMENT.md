...
批量导入通过云函数环境变量 `ADMIN_OPENIDS` 授权，不在代码中保存管理员身份。先用管理员微信账号上传一张测试图片，在 `memes` 集合中找到该记录的 `_openid`，然后在微信云开发控制台的 `memeApi` 云函数环境变量中设置：

```text
ADMIN_OPENIDS=管理员OpenID
```

多个管理员用英文逗号分隔。修改环境变量后重新部署云函数并彻底关闭、重新打开小程序。管理员每天最多批量上传和提交审核 100 张、总存储上限为 2000 张；普通账号仍最多保留 100 张。不要把真实 OpenID 写入代码、文档或 Git。
...
云函数只接受 JPG/PNG，限制为 5 MB、最长边 4096 像素、总像素 1600 万，并自动移除 EXIF/XMP/IPTC/文本块等元数据后覆盖原文件。每个 OpenID 每天最多上传 10 张/25 MB，最多保留 100 张，相同文件内容不能重复上传。

### 恶意上传熔断

普通账号每分钟最多创建 10 次、每天最多上传 10 张/25 MB、最多保留 100 张；管理员每分钟最多创建 60 次、每天最多上传 100 张/250 MB、最多保留 2000 张。同一账号上传完全相同的净化后图片仍会被拒绝，不限制不同账号使用相同图片和不同描述标签。

平台还有跨账号的每日总量熔断，默认每天 500 张或 1 GB，任一指标达到后停止当天新增上传。可在 `memeApi` 环境变量中调整：

```text
GLOBAL_DAILY_UPLOAD_COUNT=500
GLOBAL_DAILY_UPLOAD_BYTES=1073741824
```

次数只允许 100 到 10000，容量只允许 100 MB 到 20 GB；无效配置会回落到默认值。还应在云开发控制台配置资源预算告警、云存储容量告警和云函数并发上限。多微信账号协同攻击不能只靠 OpenID 限速，平台总量熔断是最后成本边界。
...
- 验证超限、重复图片和伪造格式被拒绝，带元数据图片上传后不再保留元数据
- 验证审核接口异常时内容保持非公开
- 验证举报、三人举报下架和账号封禁
- 验证其他用户无法读取私密内容
- 配置云资源告警和每日费用上限
